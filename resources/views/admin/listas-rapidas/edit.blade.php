  ...
  <form method="POST" action="{{ route('admin.listas-rapidas.update', $item->id) }}">
    ...
    <div class="row g-3">
      <div class="col-md-4">
        ...
        <input type="text" name="grupo" class="form-control" value="{{ old('grupo', $item->grupo ?? '') }}" required>
        @error('grupo')
      ...
      <div class="col-md-4">
        ...
        <input type="text" name="clave" class="form-control" value="{{ old('clave', $item->clave ?? '') }}" required>
        @error('clave')
          <small class="text-danger">{{ $message }}</small>
        @enderror
      </div>
      <div class="col-md-4">
        ...
        <input type="text" name="valor" class="form-control" value="{{ old('valor', $item->valor ?? '') }}" required>
        @error('valor')
          <small class="text-danger">{{ $message }}</small>
        @enderror
      </div>
    ...
    <div class="mt-4 d-flex gap-2">
      <button type="submit" class="btn btn-primary">
        <i class="fa-regular fa-floppy-disk me-1"></i> {{ isset($item) && $item->exists ? 'Actualizar' : 'Guardar' }}
      </button>
