...
@section('content')
  ...
  <form method="POST" action="{{ route('planificacion.clientes.store') }}">
    @csrf
    <div class="row g-3">
      <div class="col-md-6">
      ...
      <div class="col-md-4">
        ...
        <input name="documento" class="form-control" required value="{{ old('documento', $cliente->documento ?? '') }}">
      </div>
      <div class="col-md-4">
      ...
      <div class="col-md-3">
        ...
        <select name="comercial_id" class="form-select" required>
          @foreach ($comerciales as $comercial)
            <option value="{{ $comercial->id }}" @selected(old('comercial_id', $cliente->comercial_id ?? '') === $comercial->id)>
              {{ $comercial->name }}
            </option>
          @endforeach
        </select>
    ...
    <div class="mt-4 d-flex gap-2">
      ...
      <a href="{{ route('planificacion.clientes.index') }}" class="btn btn-secondary">Cancelar</a>
    </div>
  </form>
@endsection
